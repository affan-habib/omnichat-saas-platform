describe('CRUD Operations - Canned Responses', () => {
  ...
  it('should create a new canned response', () => {
    ...
    cy.contains('Establish Template').click();
  ...
  it('should delete a canned response', () => {
    ...
    cy.contains(titleToDelete).parents('.rounded-\[2\.5rem\]').within(() => {
      // Find the button with text-rose-500 class (trash icon)
      cy.get('button.text-rose-500').click({ force: true });
    });
